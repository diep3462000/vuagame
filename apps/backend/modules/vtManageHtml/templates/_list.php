<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_html" style="margin-top: 5px !important;">
      <thead>
          ...
          <?php include_partial('vtManageHtml/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $vtp_html): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $vtp_html->getId() ?>}" rel="<?php echo $vtp_html->getId() ?>">

                            <?php include_partial('vtManageHtml/list_td_batch_actions', array('vtp_html' => $vtp_html, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('vtManageHtml/list_td_tabular', array('vtp_html' => $vtp_html, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('vtManageHtml/list_td_actions', array('vtp_html' => $vtp_html, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="7">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('vtManageHtml/pagination', array('pager' => $pager)) ?></div>
