<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_advertise_images" style="margin-top: 5px !important;">
      <thead>
          ...
          <?php include_partial('vtManageAdvertises/list_th_tabular_image', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $vtp_advertise_image): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $vtp_advertise_image->getId() ?>}" rel="<?php echo $vtp_advertise_image->getId() ?>">

                            <?php include_partial('vtManageAdvertiseImages/list_td_batch_actions', array('vtp_advertise_image' => $vtp_advertise_image, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('vtManageAdvertises/list_td_tabular_image', array('vtp_advertise_image' => $vtp_advertise_image, 'orderNo' => $orderNo)) ?>
      ...
      <tfoot>
        <tr>
          <th colspan="7">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('vtManageAdvertises/pagination_image', array('pager' => $pager,'vtp_advertise' => $vtp_advertise)) ?></div>
