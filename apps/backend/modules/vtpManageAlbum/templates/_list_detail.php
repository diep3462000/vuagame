<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_services_detail" style="margin-top: 5px !important;">
      <thead>
          ...
          <?php include_partial('vtpManageAlbum/list_th_tabular_detail', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $vtp_photo): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $vtp_photo->getId() ?>}" rel="<?php echo $vtp_photo->getId() ?>">

                            <?php include_partial('vtpManagePhoto/list_td_batch_actions', array('vtp_photo' => $vtp_photo, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php  include_partial('vtpManageAlbum/list_td_tabular_detail', array('vtp_photo' => $vtp_photo, 'orderNo' => $orderNo,'vtp_album' => $vtp_album)) ?>
      ...
      <tfoot>
        <tr>
          <th colspan="7">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('vtpManageAlbum/pagination_detail', array('pager' => $pager,'vtp_album' => $vtp_album)) ?></div>
