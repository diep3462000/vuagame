<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_advertise_location" style="margin-top: 5px !important;">
      <thead>
          ...
          <?php include_partial('vtManageAdvertiseLocation/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $vtp_advertise_location): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $vtp_advertise_location->getId() ?>}" rel="<?php echo $vtp_advertise_location->getId() ?>">

                            <?php include_partial('vtManageAdvertiseLocation/list_td_batch_actions', array('vtp_advertise_location' => $vtp_advertise_location, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('vtManageAdvertiseLocation/list_td_tabular', array('vtp_advertise_location' => $vtp_advertise_location, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('vtManageAdvertiseLocation/list_td_actions', array('vtp_advertise_location' => $vtp_advertise_location, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="7">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('vtManageAdvertiseLocation/pagination', array('pager' => $pager)) ?></div>
